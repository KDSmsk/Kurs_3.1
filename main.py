...
if __name__ == '__main__':
    ...
    # Создаем список и заполняем параметры
    Dict_objs = []
    ...
    # Сортируем полученный список по дате
    Dict_objs.sort(key=sort_obj_from_date)
    #Переворачиваем список
    Dict_objs.reverse()
    Dict_obj_EXECUTED = []
    #Получаем объекты item state = "EXECUTED"
    ...
    # Далее составляем список из 5 элементов
    for index, item in enumerate(Dict_obj_resume):
        ...
        if ((index + 1) % 5) == 0:
            for item_2 in print_dict:
                ...
                # Получаем количество денег и в каой валюте
            ...
            print("**********************************************************************")
            print_dict = []
    # проверяем остаток, если больше 0 значит цифра не делится на 5 и следует вывести остатаок
    if len(print_dict) > 0:
